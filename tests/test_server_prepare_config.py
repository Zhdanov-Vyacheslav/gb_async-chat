...
from server import prepare_config

CONFIG_PATH = os.path.join(os.path.split(os.path.dirname(__file__))[0], "config.json")


class PrepareConfigTestCase(unittest.TestCase):
    def setUp(self):
    ...
    def test_none(self):
        ...
        self.assertEqual(prepare_config(options, CONFIG_PATH), self.file_config)

    def test_port_1023(self):
        options = Namespace(**{"addr": "0.0.0.0", "port": 1023})
        try:
            prepare_config(options, CONFIG_PATH)
    ...
    def test_path_bad(self):
        options = Namespace(**{"addr": "0.0.0.0", "port": None})
        try:
            prepare_config(options, "trash/path")
        except Exception as e:
